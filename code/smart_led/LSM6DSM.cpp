...
void LSM6DSM::init(uint8_t Ascale, uint8_t Gscale, uint8_t AODR, uint8_t GODR)
{
  ...
  // (can be changed by writing to bits 5, 4, and 1, resp to above register)
   ...
   // interrupt handling
   //Double-tap example
   _i2c_bus->writeByte(LSM6DSM_ADDRESS, LSM6DSM_CTRL1_XL, 0x60);       // Turn on the accelerometer  ODR_XL = 416 Hz, FS_XL = ±2 g
   _i2c_bus->writeByte(LSM6DSM_ADDRESS, LSM6DSM_TAP_CFG, 0x8E);      // Enable interrupts and tap detection on X, Y, Z-axis
   _i2c_bus->writeByte(LSM6DSM_ADDRESS, LSM6DSM_TAP_THS_6D, 0x89);    //Set tap threshold   9 * FS_XL / 25
   _i2c_bus->writeByte(LSM6DSM_ADDRESS, LSM6DSM_INT_DUR2, 0x7F);      // Set Duration, Quiet and Shock time windows
   _i2c_bus->writeByte(LSM6DSM_ADDRESS, LSM6DSM_WAKE_UP_THS, 0x80);  // Single & double-tap enabled (SINGLE_DOUBLE_TAP = 1)
   //_i2c_bus->writeByte(LSM6DSM_ADDRESS, LSM6DSM_MD1_CFG, 0x40);       // Single-tap interrupt driven to INT1 pin
   _i2c_bus->writeByte(LSM6DSM_ADDRESS, LSM6DSM_MD2_CFG, 0x08);      // Double-tap interrupt driven to INT2 pin
